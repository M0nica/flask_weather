...
def location():
    ...
    if status == 'success':
        try:
            ...
            # pass the coordinates and city name to
            # the route that gets weather info
            return redirect(url_for(
                'weather', ip_coordinates=ip_coordinates,
...
def weather(ip_coordinates, city, state):
    ...
    response = requests.get(
        ...
        + weather_key + '/' + ip_coordinates)
    data = response.json()
    # data['hourly'] contains hourly data with the time formatted as
    # Epoch Unix Time - should look into how to display hourly data in
    # weather.html
    # data['hourly']
    ...
    # print out a statement with the current weather info +
    # location that was used/detected
    # return("Right now in "+ city + ", " + state + " it is " +
    # temperature +  degree_sign + " and there  is a " + RAIN_WARNING)
    location = {'city': city, 'state': state}
    weather_info = {'temperature': temperature, 'rain': rain_commentary}
    return render_template('weather.html',
                           location=location, weather_info=weather_info,
